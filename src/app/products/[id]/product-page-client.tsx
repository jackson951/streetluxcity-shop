...
export function ProductDetailPageClient({ id }: ProductDetailPageClientProps) {
      ...
      <div className="space-y-3">
        ...
        <div className="grid grid-cols-3 gap-2">
          {(product.imageUrls?.length ? product.imageUrls : [getProductImage(product.imageUrls)]).map((img, i) => (
            <Image key={i} src={img} alt={`${product.name}-${i}`} width={320} height={160} className="h-24 w-full rounded-lg object-cover" unoptimized={true} />
          ))}
        </div>
      </div>
